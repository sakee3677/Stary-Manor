...
* **事件同步機制 (Async Event Synchronization):**
    * 解決了「對話」與「演出」不同步的問題。透過 **Coroutines** 實作了一個等待機制。
    * 當對話觸發特殊事件（如：播放一段動畫或開啟機關）時，系統會鎖定輸入，直到收到 `RegisterEventCallback()` 回傳訊號，才允許玩家繼續閱讀。
* **高度整合 UnityEvent:**
    * 利用 Unity 內建的 `UnityEvent` 系統，讓設計師可以在 Inspector 面板中直接拖曳並綁定對話結束後要觸發的邏輯（例如：`CameraShake`, `PlaySound`, `LoadScene`），無需硬編碼 (Hard-coding)。

> 🔗 **核心邏輯展示：**
> ```csharp
> // 使用 Coroutine 等待外部事件完成
...
>
>     // 暫停程式碼執行，直到收到所有事件的回調訊號 (Callback)
